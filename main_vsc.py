# python main_vsc.py  --hdim=512 --output_height=256 --channels='32, 64, 128, 256, 512, 512' --m_plus=500 --weight_rec=1 --weight_kl=1  --weight_neg=0.5 --num_vae=10000  --trainsize=74335 --test_iter=1000 --save_iter=4 --start_epoch=5040  --batchSize=30 --nrow=10 --lr_e=0.0005 --lr_g=0.0005  --cuda  --nEpochs=10000
...
parser.add_argument('--beta1', type=float, default=0.1, help='beta1 for adam. default=0.5')
...
def main():
    ...
    train_set = ImageDatasetFromFile(train_list, opt.dataroot, input_height=None, crop_height=None, output_height=opt.output_height, is_mirror=False, aug=True)
    ...
    def train_vsc(epoch, iteration, batch, denoised_batch, cur_iter):
        ...
        loss_rec =  model.reconstruction_loss(rec, denoised, True)
    ...
    def train(epoch, iteration, batch, cur_iter):  
        ...
        info = "====> Cur_iter: [{}]: Epoch[{}]({}/{}): time: {:4.4f}: ".format(cur_iter, epoch, iteration, len(train_data_loader), time.time()-start_time)
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):  
        ...
        for iteration, batch in enumerate(train_data_loader, 0):
            ...
            if epoch < opt.num_vae:
                train_vsc(epoch, iteration, batch[0], batch[1], cur_iter)
            else:
