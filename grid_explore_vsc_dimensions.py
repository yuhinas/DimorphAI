...
def grid_explore_dimensions(epoch_num=10000, model=None):

    code_dim = 512
    model_id = "model_local_epoch_10000_iter_0"

    #--------------build models -------------------------
    if model is None:
        model = VSC(cdim=3, hdim=code_dim, channels=str_to_list("32, 64, 128, 256, 512, 512"), image_size=256).cuda()
        load_model(model, f'./model/{model_id}.pth')

    model.eval()

    to_save_path = './save/%s/feature_viz' % model_id
    to_gif_save_path = './save/%s/feature_viz/gif' % model_id
    to_diff_save_path = './save/%s/feature_viz/diff' % model_id
    ...
    try:
        q005 = np.load("./save/vsc/epoch_10000/q005.npy")
    except:
...
#         images_1st_half = images[reverse_idx]
        images_1st_half = images[0:5]
        images_2nd_half = images[5:]

        save_image(images, os.path.join(to_save_path, "./feature/feature_%04d.jpg" % selected), nrow=10)
        touch_dir(os.path.join(to_save_path, 'no_neg'))
        save_image(images_1st_half, os.path.join(to_save_path, 'no_neg', "feature_%04d.jpg" % (selected + 512)), nrow=10)
        save_image(images_2nd_half, os.path.join(to_save_path, 'no_neg', "feature_%04d.jpg" % (selected)), nrow=10)
        ...
        del images_npy
        del images, images_1st_half, images_2nd_half
        del codes
